...
function setup() {
  ...
  // setup code here
  branches_ystarts = new Array(branches_amount).fill(0).map(_ => canvasHeight - randomExponential(1 / 200));
...
class Branch {
  ...
  draw() {
    ...
    for (var i = 0; i < Math.min(this.horizontal_branches_lengths.length, this.vertical_branches_lengths.length); i++) {
      ...
      col = lerpColor(this.left_color, this.right_color, this.curx / canvasWidth);
      stroke(col);
      line(this.curx, this.cury, newx, newy);
      this.curx = newx;
      this.cury = newy;
      if (this.curx > canvasWidth) {
        break;
      }
    }
  }
