 ...
 */
function startOrEndConditionColored(
    shadeSum,
    shadesAmount,
    currentPixelShade,
    ...
    //&& Math.random() < 0.95  // condition to start a new line
        ...
        // return wether we do continue on the current line
        if (shadesAmount < currentMinimumLineLength) return true;
        if (
            Math.abs(currentPixelShade[0] - shadeSum[0]) <
            shadeSum[0] / shadesAmount + randInt(0, maximumShadeMarginToBreakLine)
        ) {
            return true;
        }
        if (
            Math.abs(currentPixelShade[1] - shadeSum[1]) <
            shadeSum[1] / shadesAmount + randInt(0, maximumShadeMarginToBreakLine)
        ) {
            return true;
        }
        if (
            Math.abs(currentPixelShade[2] - shadeSum[2]) <
            shadeSum[2] / shadesAmount + randInt(0, maximumShadeMarginToBreakLine)
        ) {
            return true;
        }
        return false;
...
function drawPure(matrixToFollow) {
    ...
    for (
        var height = -Math.max(canvasHeight, canvasWidth);
        height < canvasHeight;
        ...
        while (curx < canvasWidth && cury < canvasHeight) {
            if (cury < 2 * forwardOverlapLengthAverage || curx < 2 * backwardOverlapLengthAverage) {
            ...
            if (
                ...
                shadeSum = shadeSum.map(
                    (currentShadeSum, index) => currentShadeSum + currentPixelShade[index]
                );
                shadesAmount += 1;
            } else {
                if (end != null) {
                    // draw a line between start and end and reset both values
                    let lineStart = start.translate(
                        oppositeDirectionVector,
                        randomExponentialAvg(backwardOverlapLengthAverage)
                    );
                    let lineEnd = end.translate(
                        directionVector,
