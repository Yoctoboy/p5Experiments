...
function create_waves(amount) {
  ...
  cur_x = max(0, min(0.45 * width, randomGaussian(width / 4, 150)));
  max_x = width - cur_x;

  // ensure the final drawing will be vertically centered
  y_offset = random(8, 18);
  middle_y = height / 2;
  ...
  while (cur_x < max_x) {
    ...
    y_speed = min_max_y_speed * noise_module.perlin3(0.005 * cur_x, 0.002 * y_speed, random(0, 0.002));
